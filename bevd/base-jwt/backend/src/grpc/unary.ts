...
import { actorFromMetadata } from "./metadata";
 ...
 * request id, resolve the caller from the metadata, bind a logger to both, time the call,
...
export function unary<Request, Reply>(
	...
	return (
		...
		let log = root.child({ requestId, transport: "grpc", method });
		...
		actorFromMetadata(call.metadata)
			.then((actor) => {
				...
				return handler({ db, log, actor }, call.request);
			})
			.then((reply) => {

...
import { loginSession } from "../../test/auth";
 ...
 * did not arrive fails the check and gets a 403. buildTokenCookie
 * and clearTokenCookie each take the domain separately, and passing it to only
 * one of them is the mistake this suite catches.
 *
...
describe("a second cookie of the same name", () => {
	...
	// second access_token. lacewing's readTokenCookie refuses the ambiguous
	// name; this pins the 401 and the message that separates it in the log
	// from an ordinary logged-out request.
	it("is refused, and says so distinctly", async () => {
		...
		const shadowed = await request(app)
			...
			.set(
				"Cookie",
				`access_token=planted; access_token=${session.accessToken}`
			);
		expect(shadowed.status).toBe(401);
		expect(shadowed.body.message).toBe("Ambiguous access token cookie");
	});
});

...
const envSchema = z.object({
	...
	DATABASE_URL: z
		...
		.refine((val) => /^postgres(ql)?:\/\//.test(val), {
			message:
				'DATABASE_URL must be a PostgreSQL connection string, e.g. "postgresql://user:password@localhost:5432/db?schema=public"',
		}),
	// Connections held by this process, not by the deployment: every replica
	// opens its own pool, and Postgres counts the total against max_connections.

...
import { createUser, updateUser, CreateUserData, UpdateUserData, UserData } from "../models/user.model";
...
export const updateUserHandler = (req: Request<{ id: string }>, res: Response) => {
	const user = users.get(req.params.id);
	if (!user) throw createError(404, "User not found");
